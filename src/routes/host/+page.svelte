<script>
  ...
  const roleOrder = ['mafias', 'cop', 'doctor', 'security', 'lawyer', 'maniac', 'kamikaze'];

  function initializePlayers(data) {
    const roster = createArray(data).map((card, index) => ({
      ...card,
      playerIndex: index + 1,
      alive: true,
      id: index
  ...
  function handleDrop(event, index) {
    event.preventDefault();
    const from = activeDrag !== null ? activeDrag : Number(event.dataTransfer?.getData('text/plain'));
    const to = index;
  ...
  $: scriptSteps = [
    ...
    settings.cop ? { text: `Шериф прокидається. Визначає, хто з гравців може бути мафіозі.` } : null,
    settings.doctor ? { text: `Лікар прокидається. Обирає, кого захистити цієї ночі.` } : null,
    settings.security ? { text: `Охоронець прокидається. Може захистити одного гравця, але ризикує.` } : null,
    settings.lawyer ? { text: `Адвокат прокидається. Обирає гравця для захисту від лінчу.` } : null,
    settings.maniac ? { text: `Маньяк прокидається. Обирає свою жертву серед усіх гравців.` } : null,
...
<div class="host-screen">
  <div class="host-header">
    ...
    <div class="header-actions">
      <button class="secondary-button" type="button" on:click={() => night++}>
        + Add Night
      </button>
  ...
  <div class="main-grid">
    <section class="players-panel">
      <div class="panel-title">
        ...
        <p class="hint">Drag to reorder the turn order. Toggle status to mark eliminated players.</p>
      </div>
